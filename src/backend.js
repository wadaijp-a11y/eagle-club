...
function doPost(e) {
  try {
    ...
    if (action !== 'logout') {
      const isMaintenance =
        PropertiesService.getScriptProperties().getProperty('MAINTENANCE_MODE') === 'true';
      if (isMaintenance && session.role !== '管理者') {
        // 管理者以外は強制ログアウトフラグを立てて弾く
        return jsonResponse({
          success: false,
          error: 'システムがメンテナンスモードに入りました。',
          forceLogout: true,
        });
      }
    }
...
// ==========================================
// AUTH & DASHBOARD DATA
// ログイン処理を軽くするため、全員共通のダッシュボードデータ（スケジュール・更新日）はキャッシュする
// ==========================================
function getCommonDashboardData() {
  ...
  if (histSheet) {
    ...
    if (hData.length > 1) {
      ...
      if (!isNaN(lastDate))
        eagleUpdateDateStr = Utilities.formatDate(lastDate, 'JST', 'yyyy/MM/dd');
    }
  }
  ...
  if (ohSheet) {
    ...
    if (ohData.length > 1) {
      ...
      if (!isNaN(lastDate))
        officialUpdateDateStr = Utilities.formatDate(lastDate, 'JST', 'yyyy/MM/dd');
    }
  }
...
function checkLogin(id, password) {
  ...
  for (let i = 1; i < memberData.length; i++) {
    ...
    if (isMaintenance && role !== '管理者') {
      ...
      return {
        success: false,
        error: '現在システムメンテナンス中です。終了までしばらくお待ちください。',
      };
    }
    ...
    CacheService.getScriptCache().put(token, JSON.stringify({ name, role }), SESSION_TTL_SECONDS);
...
function resetMemberPassword(targetId, newPassword) {
  ...
  if (!isValidHash(newPassword)) {
    return {
      success: false,
      error: 'パスワードの処理に失敗しました。アプリを再読み込みしてもう一度お試しください。',
    };
  }
...
【フォーマット】[{"日付":"YYYY/MM/DD","順位":1,"名前":"〇〇 〇〇","OUT":45,"IN":42,"GROSS":87,"HDCP":14,"NET":73}]`;
    ...
    if (msg.includes('quota'))
      userMsg = 'API利用上限に達しました。しばらく待って再試行してください。';
    else if (msg.includes('JSON'))
      userMsg = '画像の解析結果が不正でした。別の画像で再試行してください。';
...
function updateMemberHandicaps(scoreData) {
  ...
  for (let i = 1; i < memberData.length; i++) {
    ...
    let newHdcp = !isNaN(memberData[i][3])
      ? Number(memberData[i][3])
      : !isNaN(memberData[i][4])
        ? Number(memberData[i][4])
        : 0;

    if (compePlayers[name]) {
      const p = compePlayers[name];
      const cut = p.rank === 1 ? 0.8 : p.rank === 2 ? 0.9 : p.rank === 3 ? 0.95 : 1.0;
...
function getScoreHistory() {
  ...
  try {
    ...
    const result = data.slice(1).map(r => {
      const obj = {};
      headers.forEach((h, i) => {
        obj[h] = r[i] instanceof Date ? Utilities.formatDate(r[i], 'JST', 'yyyy/MM/dd') : r[i];
      });
      return obj;
    });
    // 取得に成功した場合のみキャッシュへ保存する（エラー時の結果はキャッシュしない）
    ...
    const result = data
      ...
      .map(r => ({
        date: Utilities.formatDate(
          r[0] instanceof Date ? r[0] : new Date(r[0]),
          'JST',
          'yyyy/MM/dd'
        ),
